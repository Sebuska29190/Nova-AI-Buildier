...
const models: ModelDef[] = [
  { id: "lmstudio-model", name: "LM Studio (loaded model)", contextWindow: 32_000, maxTokens: 4_096, cost: { input: 0, output: 0 } },
];

const plugin: ProviderPlugin = {
  id: "lmstudio", name: "LM Studio (Local)", models, auth: { method: "api-key", envVar: "LMSTUDIO_API_KEY" },
  classifyError: (e) => {
    const m = (e instanceof Error ? e.message : String(e)).toLowerCase();
    if (m.includes("401")||m.includes("auth")) return "auth";
    if (m.includes("429")||m.includes("rate")) return "rate";
    if (m.includes("refused")||m.includes("econn")||m.includes("timeout")) return "timeout";
    return "unknown";
  },
  stream: async (p: StreamParams) => {
    const base = process.env.LMSTUDIO_BASE_URL || DEFAULT_BASE;
    ...
    if (!res.ok) { p.onChunk({ type: "error", message: `LM Studio ${res.status}: ${await res.text()}` }); return; }
...
export { plugin as lmstudioProvider };
