...
export const CONTENT_NICHES: Record<string, Niche> = {
  misterio_real: {
...
const VIRAL_BOOST = new Set(["confesiones", "suspenso_cotidiano", "drama_humano", "venganza", "misterio_digital"]);

export function selectNiche(nicheName?: string): [string, Niche] {
  if (nicheName && CONTENT_NICHES[nicheName]) return [nicheName, CONTENT_NICHES[nicheName]];
  const pool = Object.keys(CONTENT_NICHES);
  const weights = pool.map((n) => VIRAL_BOOST.has(n) ? 2.0 : 1.0);
  const total = weights.reduce((a, b) => a + b, 0);
  let r = Math.random() * total;
  for (let i = 0; i < pool.length; i++) {
    r -= weights[i];
    if (r <= 0) return [pool[i], CONTENT_NICHES[pool[i]]];
  }
  return [pool[0], CONTENT_NICHES[pool[0]]];
}
